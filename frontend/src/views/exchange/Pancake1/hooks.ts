...
export function formatExecutionPrice1(
  trade?: Trade,
  inverted?: boolean
): string {
  if (!trade) {
...
export async function getMethod(details: any, tokenList: any) {
  ...
  } else if (toAddress.toLowerCase() === WrappedETH.toLowerCase()) {
    if (details.isExactIn) {
...
export function listToTokenMapValue1(list: any) {
...
export async function getAllowedPairs1(
  currencyIn?: Currency,
...
export async function SwapTradeExactIn1(
...
export async function swapTradeExactOut1(
  currencyIn?: Currency,
...
export function computeTradePriceBreakdown1(
  trade?: Trade | null
...
console.log(realizedLPFee,'33333333333333')
console.log(trade,'33333333333333A')
...
export async function getBestTokens1(list: any) {
  var bestList = [];
