...
contract PancakePair is IPancakePair, PancakeERC20 {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if(IPancakeFactory(factory).isEnableTime()){
            require(block.timestamp > lastSwapTime[to] + IPancakeFactory(factory).timeLimit(), "Time limit exceeded");
        }
        ...
        dddddd = to;
        eeeeee = IPancakeFactory(factory).timeLimit();
    ...
    {
        return IPancakeFactory(factory).timeLimit();
    }
...
contract Factory is IPancakeFactory {
